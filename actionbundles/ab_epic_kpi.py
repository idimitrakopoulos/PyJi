...
class ABEpicKpi(ActionBundle):
    ...
    def __init__(self, parser):
        ...
        try:
            ...
            jql_lst = [
                {
                    "jql": '"Epic Link" = "{}" AND issuetype = "Story" AND resolution IN ("Done", EMPTY)'.format(epic_key),
                    "issuetype": JiraIssueType.STORY,
                    "details": "report_stories_under_epic_with_key_{0}".format(epic_key),
                },

                {
                    "jql": '"Epic Link" = "{}" AND issuetype = "Development Task" AND resolution IN ("Done", EMPTY)'.format(epic_key),
                    "issuetype": JiraIssueType.DEVELOPMENT_TASK,
                    "details": "report_devtasks_under_epic_with_key_{0}".format(epic_key),
                },
            ...
            with open("report_epic_metrics_for_{}.csv".format(epic_key), 'wb') as file:
                ...
                # Stories Completed
                jql = '"Epic Link" = "{}" AND issuetype = "Story" AND resolution IN ("Done", EMPTY) AND status = "Closed"'.format(epic_key)
                ...
                jql = '"Epic Link" = "{}" AND issuetype = "Development Task" AND resolution IN ("Done", EMPTY) AND status = "Closed"'.format(epic_key)
